...
def test_value_events(mgr):
    ...
    mgr.mock_receive_json("/openzwave/1/node/2/value/3", {"value": "yo2"})
    assert len(events) == 2
    assert events[0].id == "3"
    assert events[0].value == "yo2"

    # Show how to use collection helpers
    assert mgr.get_instance("1").get_node("2").get_value("3").value == "yo2"

    # Listen for value removed
    mgr.options.listen(EVENT_VALUE_REMOVED, events.append)
    mgr.receive_message("/openzwave/1/node/2/value/3", "")
    assert len(events) == 3
    assert events[0].id == "3"
